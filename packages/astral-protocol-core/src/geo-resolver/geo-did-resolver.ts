...
import Doctype from "./geo-doc"
import type { ParsedDID, DIDResolver, DIDDocument } from "did-resolver"
import type { GeoDIDDocument } from "./geo-did-document";
...
// document wrapper 
export function wrapDocument(content: any, did: string): GeoDIDDocument {
...
export default {
    getResolver: (geo: Geo): ResolverRegistry => ({
        '3': async (did: string, parsed: ParsedDID): Promise<GeoDIDDocument | null> => {
