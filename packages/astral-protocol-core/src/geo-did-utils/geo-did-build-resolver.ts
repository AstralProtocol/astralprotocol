...
import { AstralAPI } from "../astral-client"
...
export function wrapDocument(content: any, did: string): DIDDocument {
    ...
    // how do we obtain the keyName and keyValue 
  ...
  export default {
    getResolver: (astral: Astral): ResolverRegistry => ({
      '3': async (did: string, parsed: ParsedDID): Promise<DIDDocument | null> => {
        const doctype = await astral.loadDocument(`/ceramic/${parsed.id}`)
